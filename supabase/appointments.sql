...
-- Prevent two active appointments from being booked at the exact same start time.
-- The WHERE clause excludes cancelled rows so that a cancelled slot can be re-booked.
--
-- LIMITATION: This only prevents identical start-time collisions (e.g. two bookings
-- both at 09:00). It does NOT prevent overlapping bookings (e.g. a 60-minute booking
-- at 09:00 and a 30-minute booking at 09:30 would both pass this constraint).
-- True overlap prevention requires either:
--   a) An EXCLUDE USING gist constraint with tsrange columns and the btree_gist
--      extension — recommended if you later migrate date+time to a tsrange column.
--   b) An application-level check before insert (query for active appointments
--      whose range overlaps the new slot before calling createAppointment).
-- The slot calculator already prevents overlap on the client side, so
-- this constraint is a backend safety net for the most common race condition.
